...
class GraphSmokeTests(unittest.TestCase):
    ...
    def test_not_real_derivation_run_output_dependency_does_not_satisfy_downstream(self) -> None:
        study_dir = _workspace_dir("lg2_not_real_dependency_not_runtime_evidence") / "PSY201"
        spec_dir = study_dir / "input_spec"
        run_id = "run_lg2_not_real_dependency_not_runtime_evidence"
        output_dir = study_dir / "runs" / run_id / "outputs"
        spec_dir.mkdir(parents=True)
        output_dir.mkdir(parents=True)
        adsl_path = output_dir / "adsl.csv"
        ...
        state.datasets["ADSL"].artifacts.append(
            ArtifactRef(
                artifact_id="output_adam_psy201_run_lg2_not_real_dependency_not_runtime_evidence_adsl",
                kind="output_adam",
                path=str(adsl_path.as_posix()),
                sha256=f"sha256:{sha256_file(adsl_path)}",
                dataset="ADSL",
        ...
        gateway._persist_graph_state(study_dir, state, node="test_seed_not_real_dependency_output")
        ...
        self.assertEqual(resolution["resolution_status"], "found_but_unusable")
        self.assertEqual(resolution["artifact_source"], "run_output")
        self.assertIn("not_real_derivation", resolution["reason"])
        self.assertEqual(result["blocked_datasets"][0]["dataset"], "ADAE")
        self.assertEqual(result["runnable_datasets"], [])
